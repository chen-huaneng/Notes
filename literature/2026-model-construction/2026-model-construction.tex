...
\begin{align}
    ...
    & \sum_{j \in V_1^+} x_{0j}^L = \sum_{i \in V_1^-} x_{i0'}^L = 1 \label{eq:conv-level-1-depart-arrive}\\
    ...
    % 第二级子回路消除（MTZ）
	& u_{is}^T - u_{js}^T + 1 \leq (n + 2)\left(1 - x_{ijs}^T\right), \quad \forall i \in V_s^-, \forall j \in V_s^+, i \neq j, \forall s \in S \label{eq:conv-level-2-subtour}\\
	& u_{js}^T \leq \left(n + 2\right)\sum_{\substack{i \in V_s^- \\ i \neq j}}x_{ijs}^T, \quad \forall j \in V_s^+, \ \forall s \in S \label{eq:con-level-2-subtour-2}\\
    % 无人机只能从卡车所在节点起飞
    ...
    & \rho_{ks} = \max\{\tau_{ks}^T, \tau_{ks}^D\}, \quad \forall k \in V_s^+,\ \forall s \in S \label{eq:conv-level-2-wait-time}\\
 	% 连续两次无人机飞行的路径不能重叠
 	& u_{js} - u_{is} \leq \left(n + 2\right)\delta_{ijs}, \quad \forall i, j \in N, i \neq j, \ \forall s \in S \label{eq:conv-level-2-drone-nonoverlap-1}\\
 	& u_{is} - u_{js} + 1 \leq \left(n + 2\right)\left(1 - \delta_{ijs}\right), \quad \forall i, j \in N, i \neq j, \ \forall s \in S \label{eq:conv-level-2-drone-nonoverlap-2}\\
 	& \delta_{0js} = \sum_{\substack{i \in V_s^- \\ i \neq j}} x_{ijs}^T, \quad \forall j \in V_s^+, \ \forall s \in S \label{eq:conv-level-2-drone-nonoverlap-3}\\
 	& \rho_{ls} \geq \tau_{ks}^D - M \left(3 - \sum_{\substack{j \in N \\ j \neq i, j \neq k \\ j \neq l, j \neq n}} x_{ijks}^D - \sum_{\substack{m \in N \\ m \neq l, m \neq n \\ m \neq i, m \neq k \\ m \neq j}}\sum_{\substack{n \in V_s^+ \\ n \neq i, n \neq k \\ n \neq l}} x_{lmns}^D - \delta_{ils}\right), \nonumber\\
 	& \qquad \forall i \in V_s^-, \ \forall k \in V_s^+, i \neq k, \ \forall l \in N, l \neq i, l \neq k, \ \forall s \in S \label{eq:conv-level-2-drone-nonoverlap-4}\\
    % ===== 辅助变量与决策变量定义域 =====
    & \tau_{0s}^T = 0, \quad \forall s \in S \label{eq:conv-auxiliary-initial-1}\\
    & \tau_{0s}^D = 0, \quad \forall s \in S \label{eq:conv-auxiliary-initial-2}\\
    & x_{00'}^L = x_{0',0}^L = 0 \label{eq:conv-auxiliary-initial-3}\\
    & x_{ss'}^T = x_{s',s}^T = 0, \quad \forall s \in S, \ \forall s' \in S' \label{eq:conv-auxiliary-initial-4}\\
    ...
    & \delta_{ijs} \in \{0, 1\}, \quad \forall i, j \in N \cup \{s, s'\}, \ \forall s \in S \\
