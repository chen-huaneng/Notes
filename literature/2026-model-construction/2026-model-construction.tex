...
\begin{align}
    ...
    \quad & u_j^L \geq u_i^L + \psi_{j} - M\left(1 - x_{ij}^L\right), \quad \forall i,j \in V_1, i \neq j \label{eq:conv-level-1-subtour}\\
    ...
    \quad & x_{ijs}^T \leq y_s, \quad \forall i,j \in N \cup \{s\}, \forall s \in S, i \neq j \label{eq:con-level-2-truck-store}\\
    \quad & x_{ijks}^D \leq y_s, \quad \forall i, k \in N \cup \{s\}, \forall j \in N, \forall s \in S, i \neq j, j \neq k, k \neq i \label{eq:con-level-2-drone-store}\\
	\quad & u_{is}^T - u_{js}^T + 1 \leq |N|\left(1 - x_{ijs}^T\right), \quad \forall i,j \in N, \forall s \in S, i \neq j \label{eq:conv-level-2-subtour}\\
	\quad & \sum_{\substack{j \in N \\ j \neq i}}\sum_{\substack{k \in N \cup \{s\}\\ k \neq i, k \neq j}}x_{ijks}^D \leq \sum_{\substack{h \in N \cup \{s\}\\ h \neq i}}x_{his}^T, \quad \forall i \in N \cup \{s\}, \forall s \in S \label{eq:conv-level-2-truck-drone-1}\\
	...
	\quad & \sum_{\substack{i \in N \cup \{s\}\\ i \neq j}}\sum_{\substack{k \in N \cup \{s\}\\ k \neq j, k \neq i}} q_j x_{ijks}^D \leq Q^D, \quad \forall j \in N, \forall s \in S \label{eq:conv-level-2-drone-weight}\\
	...
	\quad & \rho_{ks} \geq \tau_{ks}^D - M \left(1 - \sum_{\substack{i \in N \cup \{s\}\\ i \neq k}}\sum_{\substack{j \in N\\ j \neq k, j \neq i}}x_{ijks}^D\right), \quad \forall k \in N \cup \{s\}, \forall s \in S \label{eq:conv-level-2-wait-time-2}\\
