...
其次是顾客需求特性。任一顾客既可以由卡车配送，也可以由无人机配送，当顾客被无人机或卡车服务后，无人机和卡车都不能再次访问该顾客，无人机不能重复访问同一个顾客，即卡车不能在同一个顾客节点等待无人机，从而不会产生无人机飞行路径是个环的情况，每个顾客节点都有充足的空间供无人机起飞和降落，且无人机在会合点需要等待卡车时会降落在地面等待，因此不需要考虑无人机等待卡车的成本。可供建设的卫星仓库数量足够多以至于所有的顾客都能被服务到。
...
\begin{longtblr}[
    ...
    $\delta_{ils} \in \{0, 1\}$ & 对于卫星仓库 $s \in S$ 的两次无人机出动 $(i, j, k)$ 和 $(l, m, n)$，如果出动 $(i, j, k)$ 先于出动 $(l, m, n)$ 完成（即 $u_{ks}^T \leq u_{ls}^T$），则为1，否则为0\\
	...
	$u_{is}^T \geq 0$ & 整数辅助变量，表示卫星仓库$s \in S$的协同卡车在访问第二级网络节点 $i \in V_2$ 时的访问顺序 \\
...
\begin{align}
    ...
    \quad & \sum_{\substack{i \in V_1 \\ i \neq j}}x_{ij}^L = \sum_{\substack{k \in V_1\\ k \neq j}}x_{jk}^L = y_j, \quad \forall j \in S \label{eq:conv-level-1-flow}\\
    \quad & u_i^L - u_j^L + \psi_{j} \leq M\left(1 - x_{ij}^L\right), \quad \forall i \in V_1, \forall j \in S, i \neq j \label{eq:conv-level-1-subtour}\\
	\quad & \psi_{s} \leq u_s^L \leq Q^L, \quad \forall s \in S \label{eq:conv-level-1-weight}\\
	\quad & \sum_{s \in S}\left(\sum_{\substack{i \in N \cup \{s\} \\ i \neq j}}x_{ijs}^T + \sum_{\substack{i \in N \cup \{s\} \\ i \neq j}}\sum_{\substack{k \in N \cup \{s\} \\ k \neq i, k \neq j}}x_{ijks}^D\right) = 1, \quad \forall j \in N \label{eq:conv-level-2-customer}\\
	\quad & \sum_{i \in N}x_{sis}^T = \sum_{j \in N}x_{jss}^T = y_s, \quad \forall s \in S \label{eq:con-level-2-truck-start} \\
	\quad & \sum_{\substack{i \in N \cup \{s\}\\ i \neq j}}x_{ijs}^T = \sum_{\substack{k \in N \cup \{s\}\\ k \neq j}}x_{jks}^T, \quad \forall j \in N, \forall s \in S \label{eq:conv-level-2-flow}\\
    \quad & \sum_{i \in N \cup \{s\}}\sum_{\substack{j \in N \cup \{s\}\\ j \neq i}} x_{ijs}^T \leq M y_s, \quad \forall s \in S \label{eq:con-level-2-truck-store}\\
    \quad & \sum_{i \in N \cup \{s\}}\sum_{\substack{j \in N\\ j \neq i}}\sum_{\substack{k \in N \cup \{s\}\\ k \neq i, k \neq j}} x_{ijks}^D \leq M y_s, \quad \forall s \in S \label{eq:con-level-2-drone-store}\\
	\quad & u_{is}^T - u_{js}^T + 1 \leq (|N| + 1)\left(1 - x_{ijs}^T\right), \quad \forall i,j \in N, \forall s \in S, i \neq j \label{eq:conv-level-2-subtour}\\
	\quad & \sum_{\substack{j \in N \\ j \neq i}}\sum_{\substack{k \in N \cup \{s\}\\ k \neq i, k \neq j}}x_{ijks}^D \leq \sum_{\substack{h \in N \cup \{s\}\\ h \neq i}}x_{ihs}^T, \quad \forall i \in N \cup \{s\}, \forall s \in S \label{eq:conv-level-2-truck-drone-1}\\
	\quad & \sum_{\substack{i \in N \cup \{s\}\\ i \neq k}}\sum_{\substack{j \in N\\ j \neq k, j \neq i}}x_{ijks}^D \leq \sum_{\substack{h \in N \cup \{s\}\\ h \neq k}}x_{hks}^T, \quad \forall k \in N \cup \{s\}, \forall s \in S \label{eq:conv-level-2-truck-drone-2}\\
	\quad & \sum_{s \in S}\sum_{\substack{j \in N\\ j \neq i}}\sum_{\substack{k \in V_2\\ k \neq i, k \neq j}}x_{ijks}^D \leq 1, \quad \forall i \in V_2 \label{eq:conv-level-2-drone-takeoff}\\
	\quad & \sum_{s \in S}\sum_{\substack{i \in V_2\\ i \neq k}}\sum_{\substack{j \in N\\ j \neq k, j \neq i}}x_{ijks}^D \leq 1, \quad \forall k \in V_2 \label{eq:conv-level-2-drone-landing}\\
	\quad & u_{is}^T - u_{ks}^T + 1 \leq \left(|N| + 1\right)\left(1 - \sum_{\substack{j \in N\\ j \neq i, j \neq k}}x_{ijks}^D\right), \quad \forall i \in N \cup \{s\}, \forall k \in N, i \neq k, \forall s \in S \label{eq:conv-level-2-drone-service-order}\\
	\quad & u_{ks}^T - u_{ls}^T \leq |N|\left(3 - \sum_{\substack{j \in N\\ j \neq i, j \neq k\\ j \neq l, j \neq n}}x_{ijks}^D - \sum_{\substack{m \in N\\ m \neq l, m \neq n\\ m \neq i, m \neq k}}x_{lmns}^D - \delta_{ils}\right),\nonumber \\
	\quad & \qquad \forall i,k,l,n \in N \cup \{s\}, i \neq k, l \neq n, i \neq n, l \neq k, \forall s \in S \label{eq:conv-level-2-drone-nonoverlap-1}\\
	\quad & u_{ns}^T - u_{is}^T \leq |N|\left(2 - \sum_{\substack{j \in N\\ j \neq i, j \neq k\\ j \neq l, j \neq n}}x_{ijks}^D - \sum_{\substack{m \in N\\ m \neq l, m \neq n\\ m \neq i, m \neq k}}x_{lmns}^D + \delta_{ils}\right),\nonumber \\
	\quad & \qquad \forall i,k,l,n \in N \cup \{s\}, i \neq k, l \neq n, i \neq n, l \neq k, \forall s \in S \label{eq:conv-level-2-drone-nonoverlap-2}\\
	\quad & \sum_{j \in N}q_j\left(\sum_{\substack{i \in N \cup \{s\}\\ i \neq j}}x_{ijs}^T + \sum_{\substack{i \in N \cup \{s\}\\ i \neq j}}\sum_{\substack{k \in N \cup \{s\}\\ k \neq i, k \neq j}}x_{ijks}^D\right) + w \leq Q^T, \quad \forall s \in S \label{eq:conv-level-2-truck-weight}\\
	\quad & \psi_{s} = \sum_{j \in N}q_j\left(\sum_{\substack{i \in N \cup \{s\}\\ i \neq j}}x_{ijs}^T + \sum_{\substack{i \in N \cup \{s\}\\ i \neq j}}\sum_{\substack{k \in N \cup \{s\}\\ k \neq i, k \neq j}}x_{ijks}^D\right), \quad \forall s \in S \label{eq:conv-level-1-and-2-truck-weight}\\
	\quad & q_j \sum_{\substack{i \in N \cup \{s\}\\ i \neq j}}\sum_{\substack{k \in N \cup \{s\}\\ k \neq j, k \neq i}} x_{ijks}^D \leq Q^D, \quad \forall j \in N, \forall s \in S \label{eq:conv-level-2-drone-weight}\\
	\quad & \sum_{\substack{i \in N \cup \{s\}\\ i \neq j}}\sum_{\substack{k \in N \cup \{s\}\\ k \neq j, k \neq i}}\left(e_{ij} + e_{jk}\right)x_{ijks}^D \leq E, \quad \forall j \in N, \forall s \in S \label{eq:conv-level-2-drone-battery}\\
    % TODO: 这部分时间相关的约束可以放到拓展的内容中完成
	% \quad & \tau_{js}^T \geq \rho_{is} + \frac{d_{ij}}{v^T} - M\left(1 - x_{ijs}^T\right), \quad \forall i,j \in N \cup \{s\}, \forall s \in S, i \neq j \label{eq:conv-level-2-truck-time}\\
	% \quad & \tau_{ks}^D \geq \rho_{is} + \frac{d_{ij} + d_{jk}}{v^D} - M\left(1 - x_{ijks}^D\right), \quad \forall i,k \in N \cup \{s\}, \forall j \in N, \forall s \in S, i \neq j, j \neq k, k \neq i \label{eq:conv-level-2-drone-time}\\
	% \quad & \rho_{ks} \geq \tau_{ks}^T, \quad \forall k \in N \cup \{s\}, \forall s \in S \label{eq:conv-level-2-wait-time-1}\\
	% \quad & \rho_{ks} \geq \tau_{ks}^D, \quad \forall k \in N \cup \{s\}, \forall s \in S \label{eq:conv-level-2-wait-time-2}\\
    \quad & u_0^L = 0 \label{eq:conv-auxiliary-initial}\\
	\quad & y_s \in \{0, 1\}, \quad \forall s \in S \label{eq:conv-decision-variable}\\
	\quad & x_{ij}^L \in \{0, 1\}, \quad \forall i, j \in V_1 \\
	\quad & x_{ijs}^T \in \{0, 1\}, \quad \forall i, j \in V_2, \forall s \in S \\
	\quad & x_{ijks}^D \in \{0 ,1\}, \quad \forall i, k \in V_2, \forall j \in N, \forall s \in S \\
    \quad & \delta_{ils} \in \{0, 1\}, \quad \forall i, l \in V_2, \forall s \in S \\
    \quad & u_i^L \geq 0, \quad \forall i \in V_1 \\
    \quad & u_{is}^T \geq 0, \quad \forall i \in V_2, \forall s \in S \\
    \quad & \psi_{s} \geq 0, \quad \forall s \in S \label{eq:conv-auxiliary-variable}
    % \tau_{is}^T,\tau_{is}^D, \rho_{is} \geq 0
\end{align}
}
\end{theorem}

目标函数\cref{eq:conv-obj}最小化系统总成本，包括卫星仓库的建设成本、大型卡车的干线运输成本、协同卡车的支线运输成本以及无人机的配送成本。约束\cref{eq:conv-level-1-truck}表示只有一辆大型卡车从主仓库$0$出发进行配送且最终只有一辆大型卡车返回主仓库$0$。约束\cref{eq:conv-level-1-flow}是大型卡车的流平衡约束，并且确保只有被选中的卫星仓库才会被大型卡车访问。约束\cref{eq:conv-level-1-subtour,eq:conv-level-1-weight}是大型卡车的子回路消除约束，同时确保大型卡车在离开节点 $s$ 时的累计载荷包含了该卫星仓库的需求$\psi_{s}$，并且不超过大型卡车的最大载重$Q^L$。约束\cref{eq:conv-level-2-customer}表示所有顾客节点$j$必须被服务且仅被服务一次。约束\cref{eq:con-level-2-truck-start}表示协同卡车必须从卫星仓库出发并且最终需要返回该卫星仓库。约束\cref{eq:conv-level-2-flow}表示协同卡车在顾客节点$j$的流平衡约束。约束\cref{eq:con-level-2-truck-store,eq:con-level-2-drone-store}分别限制了没有建立的仓库不会进行卡车和无人机的配送。约束\cref{eq:conv-level-2-subtour}表示协同卡车的子路径消除约束。约束\cref{eq:conv-level-2-truck-drone-1,eq:conv-level-2-truck-drone-2}表示无人机的起飞节点$i$和降落节点$k$必须有协同卡车经过，即无人机是由协同卡车进行释放和回收的。约束\cref{eq:conv-level-2-drone-takeoff,eq:conv-level-2-drone-landing}限制了无人机在每个顾客节点和卫星仓库最多起飞和降落各一次，即无人机不会多次从同一个节点起飞。约束\cref{eq:conv-level-2-drone-service-order}表示了无人机服务的方向和卡车行驶的方向一致，即不会出现无人机从卡车后服务的节点起飞回到之前卡车服务过的节点的情况，由于约束\cref{eq:conv-level-2-drone-takeoff,eq:conv-level-2-drone-landing}的存在，隐含了$\sum_{\substack{j \in N\\ j \neq i, j \neq k}}x_{ijks}^D \leq 1$，即对任何一个顾客节点，起飞和降落的次数不能超过一次，如果使 $k \in N \cup \{s\}$ 则会导致同时存在从卫星仓库起飞和到卫星仓库降落的路径不可行，比如存在卫星仓库 $s = 1$ 、卡车路径 $ 1 \rightarrow 2 \rightarrow 1$ 和无人机路径 $1 \rightarrow 3 \rightarrow 2$，$2 \rightarrow 4 \rightarrow 1$，但由于 $u_{11}^T - u_{21}^T + 1 \leq 0$ 和 $u_{21}^T - u_{11}^T + 1 \leq 0$ 同时成立，导致该路径不可行，故令 $k \in N$。约束\crefrange{eq:conv-level-2-drone-nonoverlap-1}{eq:conv-level-2-drone-nonoverlap-2}限制了无人机起飞时不能已经在执行任务，由于每个卡车组只有一架无人机，因此无人机不能同时执行两个配送任务，即连续的两次无人机执行任务必须在前一次无人机降落之后才能进行下一次无人机的起飞，由于约束\crefrange{eq:conv-level-2-drone-takeoff}{eq:conv-level-2-drone-landing}的存在，如果 $i = l$ 或者 $k = n$，会使得 $\sum_{\substack{j \in N\\ j \neq i, j \neq k}}x_{ijks}^D + \sum_{\substack{m \in N\\ m \neq l, m \neq n}}x_{lmns}^D \leq 1$，因此不需要在该约束上加上 $i \neq l, k \neq n$ 的限制。约束\cref{eq:conv-level-2-truck-weight}表示协同卡车的总载重约束，即卡车配送的货物和无人机配送的货物以及无人机自身的重量不能超过卡车的最大载重量$Q^T$。约束\cref{eq:conv-level-1-and-2-truck-weight}是需求传递约束，用于确定卫星仓库 $s \in S$ 的卡车带出的货物总重量 $\psi_{s}$。约束\cref{eq:conv-level-2-drone-weight}表示无人机每次飞行的货物载重约束限制，约束\cref{eq:conv-level-2-drone-battery}表示无人机每次飞行的电量消耗约束限制。约束\cref{eq:conv-auxiliary-initial}初始化变量，约束\crefrange{eq:conv-decision-variable}{eq:conv-auxiliary-variable}表示变量的定义域。

% 约束\cref{eq:conv-level-2-truck-time}表示卡车在节点之间的行驶时间推移，到达 $j$ 的时间必定晚于离开 $i$ 的时间加上行驶时间，同理约束\cref{eq:conv-level-2-drone-time}表示无人机的飞行时间推移。约束\cref{eq:conv-level-2-wait-time-1,eq:conv-level-2-wait-time-2}表示无人机和卡车的等待机制，即卡车组离开节点 $k$ 的时间 $\rho_{ks}$ 必须同时晚于卡车到达 $k$ 的时间以及无人机到达 $k$的时间。
