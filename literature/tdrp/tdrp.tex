...
\title{Truck and Drone collaborative Routing Problem}
\author{Chen Huaneng}
\date{\today}
...
\part{Traveling Salesman Problem with Drone}
...
FSTSP数学模型可以表示为MILP \crefrange{eq:fstsp-obj}{eq:fstsp-drone-order-bound}。
...
\begin{align}
    ...
    \quad & 
        \sum_{j\in N_{+}}x_{0j}=1\label{eq:fstsp-truck-start}\\
    \quad & 
        \sum_{i\in N_{0}}x_{i,c + 1}=1\label{eq:fstsp-truck-return}\\
    \quad & 
        u_i - u_j + 1 \leq (c + 2)(1 - x_{ij}), \quad \forall i \in C, j \in \{N_{+}:j \neq i\}\label{eq:fstsp-truck-mtz}\\
    \quad & 
        \sum_{\substack{i\in N_{0}\\ i \neq j}}x_{ij}=\sum_{\substack{k\in N_{+}\\k\neq j}}x_{jk}, \quad \forall j \in C\label{eq:fstsp-truck-continuity}\\
    \quad & 
        \sum_{\substack{j\in C\\j \neq i}}\!\sum_{\substack{k\in N_{+}\\\langle i,j,k \rangle\in P}}y_{ijk}\leq1, \quad \forall i\in N_{0}\label{eq:fstsp-drone-land}\\
    \quad & 
        \sum_{\substack{i\in N_{0}\\i\neq k}}\!\sum_{\substack{j\in C\\\langle i,j,k \rangle\in P}}y_{ijk}\leq1, \quad \forall k \in N_{+}\label{eq:fstsp-drone-takeoff}\\
    \quad & 
        2 y_{ijk} \leq \sum_{\substack{h \in N_0 \\ h \neq i}}x_{hi} + \sum_{\substack{l \in C\\ l \neq k}}x_{lk}, \quad \forall i \in C, j \in \{C: j \neq i\}, k \in \{N_{+}: \langle i,j,k \rangle \in P\}\label{eq:fstsp-drone-truck-meet}\\
    \quad & 
        y_{0jk} \leq \sum_{\substack{h \in N_0 \\ h \neq k}}x_{hk}, \quad \forall j \in C, k \in \{N_{+}: \langle 0,j,k \rangle \in P\}\label{eq:fstsp-drone-truck-meet-begin}\\
    \quad & 
        u_k - u_i \geq 1 - (c+2)\left(1 - \sum_{\mySubstack{j \in C \\ \langle i,j,k \rangle \in P}}y_{ijk}\right), \quad \forall i \in C, k \in \{N_{+}: k \neq i\}\label{eq:fstsp-truck-visit-order}\\
    \quad & 
        t_i' \geq t_i - M\left(1 - \sum_{\substack{j \in C\\ j \neq i}}\sum_{\substack{k \in N_{+}\\ \langle i,j,k \rangle \in P}}y_{ijk}\right), \quad \forall i \in C\label{eq:fstsp-truck-drone-takeoff-time-1}\\
    \quad & 
        t_i' \leq t_i + M\left(1 - \sum_{\substack{j \in C\\ j \neq i}}\sum_{\substack{k \in N_{+}\\ \langle i,j,k \rangle \in P}}y_{ijk}\right), \quad \forall i \in C\label{eq:fstsp-truck-drone-takeoff-time-2}\\
    \quad & 
        t_k' \geq t_k - M\left(1 - \sum_{\substack{i \in N_0\\ i \neq k}}\sum_{\substack{j \in C\\ \langle i,j,k \rangle \in P}}y_{ijk}\right), \quad \forall k \in N_{+}\label{eq:fstsp-truck-drone-land-time-1}\\
    \quad & 
        t_k' \leq t_k + M\left(1 - \sum_{\substack{i \in N_0\\ i \neq k}}\sum_{\substack{j \in C\\ \langle i,j,k \rangle \in P}}y_{ijk}\right), \quad \forall k \in N_{+}\label{eq:fstsp-truck-drone-land-time-2}\\
    \quad & 
    \begin{aligned}
        &
    ...
    \end{aligned}\label{eq:fstsp-drone-sortie-order}\\
...
目标函数\eqref{eq:fstsp-obj}追求最小化卡车到达终点仓库$c+1$的有效时间$t_{c+1}$，通过约束\eqref{eq:fstsp-truck-drone-land-time-1}和\eqref{eq:fstsp-truck-drone-land-time-2}来对齐无人机和卡车最晚到达终点仓库的时间，所以目标函数相当于$\min\{\max\{t_{c+1},t_{c+1}'\}\}$。

参考\href{https://zhuanlan.zhihu.com/p/3235861366}{知乎解读文章}，可以将约束条件可以分为四类：

\begin{itemize}
    \item 客户有关的约束：约束\eqref{eq:fstsp-customer-visit}要求对于任何一位顾客$j$，必须且只能被卡车（或无人机）服务一次。
    \item 卡车有关的约束：
    \begin{itemize}
        \item 卡车流平衡约束：约束\eqref{eq:fstsp-truck-start}要求卡车必须从起点仓库流出一次，约束\eqref{eq:fstsp-truck-return}要求卡车必须从终点仓库流入一次，约束\eqref{eq:fstsp-truck-continuity}要求卡车在中间节点满足流入和流出相等的流平衡约束。
        \item 卡车破子圈约束：约束\eqref{eq:fstsp-truck-mtz}是MTZ形式的破子圈约束\footnote{关于破子圈约束可以参考\href{https://mp.weixin.qq.com/s/i7I-o0LiC_JP3vVOQw2AIw}{TSP中两种不同消除子环路的方法}和\href{https://mp.weixin.qq.com/s/tDYOxlSQHKRJkf5EcaBJ1A}{浅谈旅行商问题（TSP）的七种整数规划模型}}，去除了子圈存在的可能，这里$M$取到了$u_i - u_j + 1$的上界$c+2$，$u_i$可以理解为点$i$的访问次序，比如$u_1 = 5$可以理解为点1是从出发点开始，第五个被访问到的点。
    \end{itemize}
    \item 无人机有关的约束：
    \begin{itemize}
        \item 无人机发射、回收节点流约束：约束\eqref{eq:fstsp-drone-land}表示无人机可以从非终点仓库流出，约束\eqref{eq:fstsp-drone-takeoff}表示无人机可以从非起点仓库流入。
        \item 无人机访问、回收节点时间戳约束：约束\eqref{eq:fstsp-drone-time-fly}表示无人机访问顾客的时间戳应该符合时间逻辑，即不早于起飞时间戳$t_i'$+前往服务顾客点的飞行时长$\tau_{ij}'$\footnote{起飞时间$s_L$没有被包含进来的原因是，当无人机从顾客点$i$起飞时，约束\eqref{eq:fstsp-truck-drone-takeoff-time-1}和\eqref{eq:fstsp-truck-drone-takeoff-time-2}会使得$t_i' = t_i$，然后约束\eqref{eq:fstsp-truck-drone-land-time-1}和\eqref{eq:fstsp-truck-drone-land-time-2}会将起飞的时间$s_L$包含在飞行到顾客点$j$的时间内。}，约束\eqref{eq:fstsp-drone-time-fly-back}表示无人机回到卡车的时间戳应该符合时间逻辑，即不早于访问顾客点的$t_j'$+返回卡车的飞行时长$\tau_{jk}'$+回收无人机用时$s_R$\footnote{这里降落的时间$s_R$必须被包含进来的原因是，卡车可能比无人机更快到达会合点$k$（原文中举出了一个例子以助于理解\cite{murrayFlyingSidekickTraveling2015}）。}。
        \item 无人机电量续航约束：约束\eqref{eq:fstsp-drone-endurance}表示无人机的飞行时间不能超过其续航时间，即到达会合点$t_k'$的有效时间$-$无人机从节点$i$的起飞时间$(t_j'-\tau_{ij}')$（不直接使用$t_i'$是因为$t_i'$不是起飞的时间戳而是无人机到达$i$点的时间戳）要在无人机的续航时间$e$之内。
        \item 无人机飞行次序约束：约束\eqref{eq:fstsp-drone-sortie-order}要求无人机对于任意两条路径$\langle i,j,k \rangle$和$\langle l,m,n \rangle$而言，如果无人机先从节点$i$起飞之后的某个时间才从节点$l$（$p_{il}=1$）起飞，则无人机必须先完成上一次飞行才能继续下一次飞行（$t_l'\geq t_k'$），并且任意两条路径之间无交叉。
    \end{itemize}
    \item 无人机和卡车同步有关的约束：
    \begin{itemize}
        \item 无人机发射、回收点卡车访问约束：约束\eqref{eq:fstsp-drone-truck-meet}要求对于非起点发射的无人机（$\forall i \in C$），卡车必须经过无人机的起飞点$i$和降落点$k$，约束\eqref{eq:fstsp-drone-truck-meet-begin}要求对于从起点仓库起飞的无人机来说，卡车必须经过无人机的降落点。
        \item 无人机访问顾客时卡车访问次序约束：约束\eqref{eq:fstsp-truck-visit-order}要求卡车必须先访问无人机的起飞点再访问无人机的降落点。
        \item 无人机发射点时间戳约束：约束\eqref{eq:fstsp-truck-drone-takeoff-time-1}和\eqref{eq:fstsp-truck-drone-takeoff-time-2}为无人机发射点的有效时间约束，要求无人机在发射节点的有效时间等于卡车在该点的有效时间，共同实现了卡车和无人机在发射节点时间上的对齐。
        \item 无人机回收点时间戳约束：约束\eqref{eq:fstsp-truck-drone-land-time-1}和\eqref{eq:fstsp-truck-drone-land-time-2}为无人机回收点的有效时间约束，要求无人机在回收节点的有效时间等于卡车在该点的有效时间，共同实现了卡车和无人机在回收节点时间上的对齐。
        \item 卡车访问顾客节点时间戳约束：约束\eqref{eq:fstsp-truck-time}要求卡车访问当前顾客点$k$时必须要先将需要起飞的无人机$s_L$发射或者需要降落的无人机$s_R$回收，并且要大于到达顾客点$h$的有效时间戳$t_h$+路径$\langle h,k \rangle$所花费的时间$\tau_{hk}$\footnote{这里假设卡车从$h \in N_0$行驶到$k \in N_+$。}。
    \end{itemize}
    \item 辅助变量和决策变量：
    \begin{itemize}
        \item 卡车访问次序约束：约束\eqref{eq:fstsp-drone-truck-aide-1}，\eqref{eq:fstsp-drone-truck-aide-2}和\eqref{eq:fstsp-drone-order}决定了卡车访问次序$p_{ij}$取值的合理性，$u_i$和$p_{ij}$主要用于约束被卡车访问的节点之间的次序，对于被无人机服务的顾客点$i$或者$j$来说，$u_i$和$p_{ij}$的取值并不重要。
        \item 辅助变量及决策变量的初始值和取值范围：约束\eqref{eq:fstsp-truck-start-time}和\eqref{eq:fstsp-drone-start-time}给定了卡车和无人机有效时间的初始值，约束\eqref{eq:fstsp-drone-start-order}规定起点仓库的访问次序一定在其他所有顾客节点之前，约束 \eqref{eq:fstsp-x-bound} 和 \eqref{eq:fstsp-y-bound} 给定了决策变量的取值范围，约束\eqref{eq:fstsp-u-bound}规定了卡车破子圈辅助变量的取值范围，约束\eqref{eq:fstsp-truck-time-bound}和\eqref{eq:fstsp-drone-time-bound}规定了卡车和无人机的有效时间必须是非负实数，约束\eqref{eq:fstsp-drone-order-bound}给定了卡车访问次序辅助变量的取值范围。
    \end{itemize}
\end{itemize}

在约束\crefrange{eq:fstsp-truck-drone-takeoff-time-1}{eq:fstsp-drone-endurance}中，$M\geq \max\{t_{c+1}, t_{c+1}'\}$取一个非常大的数，需要大于等于最后到达终点仓库的卡车（无人机）的有效时间。由于无法事先确定最小可接受的$M$值，因此一种方法是用nearest neighbor heuristic来计算一个访问所有顾客节点并返回仓库的时间上限。算法的大致过程：初始化$M = 0$，然后从仓库开始构建卡车路径（$i=0$），找到最近的还未访问过的顾客节点$j$，让$M \gets M + \tau_{ij}$，更新$i = j$然后重复这个过程，即不断添加距离最近的未访问过的顾客节点直到所有的顾客都被访问一遍，最终让$M \gets M+\tau_{i,c+1}$，即让卡车返回仓库。
...
TDTL数学模型可以表示为MILP \crefrange{eq:tdtl-obj}{eq:tdtl-battery-limit-d}。

\begin{theorem}
\begin{align}
    \min \quad & s_{e} \label{eq:tdtl-obj}\\
    \text{s.t.} \quad & \sum_{j \in \delta^{-}(i)} u_{ji} \leq 1, \quad \forall i \in N \setminus \{o, e\} \label{eq:tdtl-truck-visit-limit}\\
    & \sum_{j \in \delta^{+}(i)} u_{ij} - \sum_{j \in \delta^{-}(i)} u_{ji} = 0, \quad \forall i \in N \setminus \{o, e\} \label{eq:tdtl-truck-flow-balance}\\
    & \sum_{j \in \delta^{+}(o)} u_{oj} = 1 \label{eq:tdtl-truck-start}\\
    & \sum_{i \in \delta^{-}(e)} u_{ie} = 1 \label{eq:tdtl-truck-end}\\
    & \sum_{j \in \delta^{-}(i)} v_{ji} \leq 1, \quad \forall i \in N \setminus \{o, e\} \label{eq:tdtl-drone-visit-limit}\\
    & \sum_{j \in \delta^{+}(i)} v_{ij} - \sum_{j \in \delta^{-}(i)} v_{ji} = 0, \quad \forall i \in N \setminus \{o, e\} \label{eq:tdtl-drone-flow-balance}\\
    & \sum_{j \in \delta^{+}(o)} v_{oj} = 1 \label{eq:tdtl-drone-start}\\
    & \sum_{i \in \delta^{-}(e)} v_{ie} = 1 \label{eq:tdtl-drone-end}\\
    & \sum_{i \in \delta^{-}(j)} u_{ij} + \sum_{i \in \delta^{-}(j)} v_{ij} \geq 1, \quad \forall j \in N \setminus \{o\} \label{eq:tdtl-visit-either}\\
    & s_{j} \geq s_{i} + t_{ij}^{T} \cdot u_{ij} - M \cdot (1 - u_{ij}), \quad \forall (i, j) \in A \label{eq:tdtl-truck-time}\\
    & s_{j} \geq s_{i} + t_{ij}^{D} \cdot v_{ij} - M \cdot (1 - v_{ij} + u_{ij}), \quad \forall (i, j) \in A \label{eq:tdtl-drone-time}\\
    & s_{o} = 0 \label{eq:tdtl-start-time}\\
    & b_{j}^{-} \leq Q + M \cdot (2 - v_{ij} - u_{ij}), \quad \forall (i, j) \in A \label{eq:tdtl-battery-limit-a-1}\\
    & b_{j}^{-} \geq Q - M \cdot (2 - v_{ij} - u_{ij}), \quad \forall (i, j) \in A \label{eq:tdtl-battery-limit-a-2}\\
    & b_{j}^{+} \leq Q + M \cdot (2 - v_{ij} - u_{ij}), \quad \forall (i, j) \in A \label{eq:tdtl-battery-limit-a-3}\\
    & b_{j}^{+} \geq Q - M \cdot (2 - v_{ij} - u_{ij}), \quad \forall (i, j) \in A \label{eq:tdtl-battery-limit-a-4}\\
    & b_{j}^{-} \leq b_{i}^{+} - t_{ij}^{D} + M \cdot \left(1 - v_{ij} + u_{ij} + \sum_{k \neq i} u_{kj}\right), \quad \forall (i, j) \in A \label{eq:tdtl-battery-limit-b-1}\\
    & b_{j}^{-} \geq b_{i}^{+} - t_{ij}^{D} - M \cdot \left(1 - v_{ij} + u_{ij} + \sum_{k \neq i} u_{kj}\right), \quad \forall (i, j) \in A \label{eq:tdtl-battery-limit-b-2}\\
    & b_j^+ \leq b_j^- + M \cdot \left(1 - v_{ij} + u_{ij} + \sum_{k \neq i}u_{kj}\right), \quad (i, j) \in A\label{eq:tdtl-battery-limit-b-3}\\
    & b_j^+ \geq b_j^- - M \cdot \left(1 - v_{ij} + u_{ij} + \sum_{k \neq i}u_{kj}\right), \quad (i, j) \in A\label{eq:tdtl-battery-limit-b-4}\\
    & b_j^- \leq b_j^+ -t_{ij}^D + M \cdot \left(1 - v_{ij} + u_{ij} + 1 - \sum_{k \neq i}u_{kj}\right), \quad (i, j) \in A\label{eq:tdtl-battery-limit-c-1}\\
    & b_j^- \geq b_j^+ -t_{ij}^D - M \cdot \left(1 - v_{ij} + u_{ij} + 1 - \sum_{k \neq i}u_{kj}\right), \quad (i, j) \in A\label{eq:tdtl-battery-limit-c-2}\\
    & b_j^+ \leq Q + M \cdot \left(1 - v_{ij} + u_{ij} + 1 - \sum_{k \neq i}u_{kj}\right), \quad (i, j) \in A\label{eq:tdtl-battery-limit-c-3}\\
    & b_j^+ \geq Q - M \cdot \left(1 - v_{ij} + u_{ij} + 1 - \sum_{k \neq i}u_{kj}\right), \quad (i, j) \in A\label{eq:tdtl-battery-limit-c-4}\\
    & b_0^+ = Q\label{eq:tdtl-battery-limit-d}
\end{align}
\end{theorem}

目标函数\eqref{eq:tdtl-obj}追求最小化服务完所有顾客并到达终止节点的时间$s_e$。

\begin{itemize}
    \item 卡车相关的路径约束：约束\eqref{eq:tdtl-truck-visit-limit}限制每个顾客节点最多只能被卡车服务一次；约束\eqref{eq:tdtl-truck-flow-balance}要求卡车在顾客节点满足流入和流出相等的流平衡；约束\eqref{eq:tdtl-truck-start}和\eqref{eq:tdtl-truck-end}分别要求卡车只能在起始节点流出一次，只能在终止节点流入一次。
    \item 无人机相关的路径约束：约束\eqref{eq:tdtl-drone-visit-limit}限制每个顾客节点最多只能被无人机服务一次；约束\eqref{eq:tdtl-drone-flow-balance} 要求无人机在顾客节点满足流入和流出相等的流平衡；约束\eqref{eq:tdtl-drone-start}和\eqref{eq:tdtl-drone-end}分别要求无人机只能在起始节点流出一次，只能在终止节点流入一次。
    \item 节点访问约束：约束\eqref{eq:tdtl-visit-either}要求每个节点都必须被卡车或无人机（或者两者一起）访问过一次。
    \item 同步相关的约束：约束\eqref{eq:tdtl-truck-time}和\eqref{eq:tdtl-drone-time}计算了每个节点的离开时间；约束\eqref{eq:tdtl-start-time}规定了离开起始节点的时间。
    \item 无人机电量相关约束：约束\eqref{eq:tdtl-battery-limit-d}表示无人机离开起始节点$o$时的电量为$Q$。与无人机电量相关的约束根据无人机的状态不同可以分为三种类型，如图\eqref{fig:tdtl-drone-battery}所示。
    \begin{itemize}
        \item[a)] 卡车搭载无人机：约束\crefrange{eq:tdtl-battery-limit-a-1}{eq:tdtl-battery-limit-a-4}表示当无人机和卡车同时访问节点$j$时，即卡车搭载无人机进行服务，无人机在到达节点$j$和离开节点$j$时的状态都是充满电的状态（$b_j^-=b_j^+=Q$），即卡车搭载无人机访问时无人机不消耗电量。
        \item[b)] 无人机单独服务节点：约束\crefrange{eq:tdtl-battery-limit-b-1}{eq:tdtl-battery-limit-b-4}表示当无人机单独访问节点$j$时，无人机在到达节点$j$的电量应该是无人机在离开节点$i$的电量减去路径$(i, j)$所消耗的电量（$b_j^-=b_i^+-T_{ij}^D$），当无人机离开节点$j$时的电量应该等于无人机到达节点$j$的电量（$b_j^+=b_j^-$），因为假设无人机在降落和起飞过程中不消耗电量。
        \item[c)] 无人机与卡车会合：约束\crefrange{eq:tdtl-battery-limit-c-1}{eq:tdtl-battery-limit-c-4}表示当无人机与卡车在节点$j$会合时，无人机到达节点$j$时的电量应该是$b_j^- = b_i^+ - T_{ij}^D$，当无人机与卡车会合时，无人机会在卡车上通过替换电池来将电量充满，因此当无人机离开节点$j$时的电量应该是$b_j^+ = Q$。
    \end{itemize}
\end{itemize}
...
% {
% \begin{align}
%     \text{s.t.} \quad & \sum_{(i,j) \in E_{rlm}}F_{ij} \leq P, \quad \forall m \in R_{rl} \label{eq:2e-rp-td-drone-battery-limit}\\
%     \quad & \sum_{i \in V_{rlm}}w_i \leq W_H, \quad \forall m \in R_{rl} \label{eq:2e-rp-td-drone-payload-limit}\\
% \end{align}
% }
%
%
% \begin{itemize}
%     \item 无人机相关的约束：约束\eqref{eq:2e-rp-td-drone-battery-limit}要求无人机在伴随子路径 $m$ 行驶过程的能源总消耗不超过无人机的总电量，约束\eqref{eq:2e-rp-td-drone-payload-limit}要求无人机在伴随路径 $m$ 行驶过程中的运输包裹总重量不超过无人机的最大载重量。 
% \end{itemize}
...
\begin{table}[!htbp]
    ...
    \begin{tabularx}{\textwidth}{lX}
        ...
        $t(\mathscr{T}_k)$ & 卡车在 $OP_k$ 中完成其对应任务所需的时间，计算公式为\eqref{eq:multi-drop-fstsp-operation-time-2} \\
        $t(\mathscr{D}_k)$ & 无人机在 $OP_k$ 中完成其对应任务所需的时间，计算公式为\eqref{eq:multi-drop-fstsp-operation-time-3} \\
        $t_k = t(OP_k)$ & $OP_k$ 的完成时间($OP_k$的成本函数)，计算公式为\eqref{eq:multi-drop-fstsp-operation-time-1} \\
...
multi-drop FSTSP 数学模型可以表示为MILP \crefrange{eq:multi-drop-fstsp-obj}{eq:multi-drop-fstsp-y-limit}。

\begin{theorem}
\begin{align}
    \min \quad & M = \sum_{k \in K}t_kx_k \label{eq:multi-drop-fstsp-obj}\\
    \text{s.t.} \quad & \sum_{k \in K(v)}x_k \geq 1, \quad \forall v \in V \setminus \{0\}\label{eq:multi-drop-fstsp-customer-visit}\\
    \quad & \sum_{k \in K^{\mathscr{D}}(v)}x_k \leq 1, \quad \forall v \in Vd \setminus \{0\}\label{eq:multi-drop-fstsp-customer-visit-drone}\\
    \quad & \sum_{k \in K^{\mathscr{T}}(v)}x_k \leq 2\left( 1 - \sum_{k \in K^{\mathscr{D}}(v)}x_k \right), \quad \forall v \in Vd \setminus \{0\}\label{eq:multi-drop-fstsp-customer-visit-limit}\\
    \quad & \sum_{k \in K^+(v)}x_k \leq n Y_v, \quad \forall v \in V\label{eq:multi-drop-fstsp-start-end-limit}\\
    \quad & \sum_{k \in K^+(v)}x_k = \sum_{k \in K^-(v)}x_k, \quad \forall v \in V\label{eq:multi-drop-fstsp-hamiltonian-1}\\
    \quad & \sum_{k \in K^+(S)}x_k \geq Y_v, \quad \forall S \subset V \setminus \{0\}, v \in S\label{eq:multi-drop-fstsp-hamiltonian-2}\\
    \quad & \sum_{k \in K^+(0)}x_k \geq 1\label{eq:multi-drop-fstsp-hamiltonian-3}\\
    \quad & \tau_{i \to D_1}^{\mathscr{D}} + \sum_{i = 1}^{\lvert D\rvert -1}\tau_{D_i \to D_{i+ 1}}^{\mathscr{D}} + \tau_{\lvert D\rvert \to j}^{\mathscr{D}} + w_r^{\mathscr{D}} \leq \epsilon \label{eq:multi-drop-fstsp-drone-battery-limit}\\
...
目标函数\eqref{eq:multi-drop-fstsp-obj}旨在最小化总路径的完成时间。

\begin{itemize}
    \item 顾客相关的约束：约束\eqref{eq:multi-drop-fstsp-customer-visit}确保所有的节点都被至少访问过一次；约束\eqref{eq:multi-drop-fstsp-customer-visit-drone}确保所有可以被无人机服务的顾客点最多被无人机服务一次；约束\eqref{eq:multi-drop-fstsp-customer-visit-limit}确保被无人机服务过的节点不会被卡车服务，反之亦然，系数2是由于当无人机没有服务过节点 $v$ 时，节点 $v$ 可以作为卡车某个操作的终点及另一个操作的起点（即在节点$v$最多在两个操作中存在，一次作为操作的起点，一次作为操作的终点）；约束\eqref{eq:multi-drop-fstsp-start-end-limit}表示如果节点 $v$ 是某个操作的终止节点，那么节点 $v$ 一定要是某个操作的起始节点。 
    \item \href{https://www.wikiwand.com/en/articles/Hamiltonian\%20path}{Hamilton Graph}约束：约束\eqref{eq:multi-drop-fstsp-hamiltonian-1}确保以节点 $v$ 为终止点的操作接下来的操作会以节点 $v$ 为起始点（流量平衡约束）；约束\eqref{eq:multi-drop-fstsp-hamiltonian-2}确保任何非空的客户子集$S$必须至少有一个操作从外部进入，即如果节点 $v$ 被选为某个操作的起始节点($Y_v = 1$)，则必须存在至少一个操作进入包含$v$的子集$S$，避免子回路的产生（消除子回路约束）；约束\eqref{eq:multi-drop-fstsp-hamiltonian-3}确保终止点为仓库的操作只有一次，这是因为如果终止点为仓库的操作大于1次，由于约束\eqref{eq:multi-drop-fstsp-customer-visit-limit}限制了卡车和无人机服务节点的次数，并且约束\eqref{eq:multi-drop-fstsp-customer-visit}确保每个节点都被访问过至少一次，而目标函数是最小化时间成本，如果多次以仓库为终止点的操作存在，则会增加成本，因此终止节点为仓库的操作大于1次的情况会被过滤掉。
    \item 无人机相关的约束：约束\eqref{eq:multi-drop-fstsp-drone-battery-limit}要求无人机在一次sortie过程中消耗的电量不能超过其续航限制。
    \item 决策变量和辅助决策变量及初始值：约束\eqref{eq:multi-drop-fstsp-start}通过确保仓库被作为起点访问使得仓库作为路径的起始点；约束\eqref{eq:multi-drop-fstsp-x-limit}和\eqref{eq:multi-drop-fstsp-y-limit}定义了决策变量 $x_k$ 和辅助决策变量 $Y_v$ 的取值范围。 
\end{itemize}

Windras Mara等(2022)\cite{windrasmaraAdaptiveLargeNeighborhood2022}用Adaptive Large Neighborhood Search (ALNS)来解决该问题，文章中使用到的ALNS算法符号及含义如表\ref{tab:multi-drop-fstsp-alns-sign-meaning}所示。
...
\begin{table}[!htbp]
    ...
    \begin{tabularx}{\textwidth}{lX}
        ...
        $\Pr$ & 选择的概率，计算公式为\eqref{eq:multi-drop-fstsp-ac}\\
        $T_c$ & 当前的温度 \\
        $T_1$ & 初始的温度 \\
        $T_0$ & 最终的温度 \\ 
        $\alpha$ & 冷却系数\\ 
        ...
        $P$ & 概率\\
        $f_c$ & 递减线性函数\\ 
        $ND_c$ & 被 destroyed 的顾客节点数量，由\eqref{eq:multi-drop-fstsp-destroy-schedule-1}和\eqref{eq:multi-drop-fstsp-destroy-schedule-2}决定\\ 
...
Parallel Drone Scheduling Traveling Salesman Problem (PDSTSP) 同样由 Murray 等(2015)\cite{murrayFlyingSidekickTraveling2015}提出。该模型适用于大量的顾客节点在无人机直接从仓库起飞的续航里程范围内。PDSTSP描述：一辆卡车和一群（单个或者多个都可以）完全相同的无人机从单个仓库出发分别服务顾客，每个顾客只能被服务一次，卡车遵循TSP路径服务，无人机直接从仓库起飞服务顾客，不同于FSTSP，PDSTSP中的无人机不需要和卡车进行会合。PDSTSP的目标是最小化最终到达仓库的卡车（无人机）的时间。
...
PDSTSP数学模型可以表示为MILP \crefrange{eq:pdstsp-obj}{eq:pdstsp-y-bound}。
...
目标函数\eqref{eq:pdstsp-obj}追求最小化完工时间$z$，即无人机和卡车最晚到达终点仓库的时间，通过约束\eqref{eq:pdstsp-truck-min-time}和\eqref{eq:pdstsp-drone-min-time}分别限制卡车和无人机最晚到达终点仓库的时间来实现；约束\eqref{eq:pdstsp-customer}确保了每个顾客能且只能被服务一次，服务可以由无人机或者卡车提供；约束\eqref{eq:pdstsp-truck-start}和\eqref{eq:pdstsp-truck-end}要求卡车必须从起点仓库$0$出发并返回终点仓库$c+1$一次，约束\eqref{eq:pdstsp-truck-flow}要求卡车在中间的顾客节点满足流入和流出相等的流约束；约束\eqref{eq:pdstsp-mtz}是MTZ形式的破子圈约束；约束\eqref{eq:pdstsp-u-bound}，\eqref{eq:pdstsp-x-bound}和\eqref{eq:pdstsp-y-bound}给出了决策变量和辅助决策变量的取值范围。
...
\part{multiple Traveling Salesman Problem with Drones}
\section{multiple Traveling Salesman Problem with Drones}
...
\part{Vehicle Routing Problem with Drones}
\section{Vehicle Routing Problem with Drones}
...
\part{Truck Support Drone Delivery}
\section{Truck Support Drone Delivery}
