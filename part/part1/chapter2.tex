...
\begin{align}
    ...
    \quad & \sum_{j \in N_{+}}\hat{x}_{0,j} = 1\label{eq:pdstsp-}
\end{align}
\end{model}
}

约束\ref{eq:pdstsp-obj}追求最小化完工时间$z$，即无人机和卡车最晚到达终点仓库的时间，通过约束\ref{eq:pdstsp-truck-min-time}和\ref{eq:pdstsp-drone-min-time}分别限制卡车和无人机最晚到达终点仓库的时间来实现；约束\ref{eq:pdstsp-customer}确保了每个顾客能且只能被服务一次，服务可以由无人机或者卡车提供；
